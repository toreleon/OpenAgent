...
- For named LOGINS, use \`Sites.account.signup(username, password)\`, \`Sites.account.login(username, password)\`, \`Sites.account.logout()\`, and \`Sites.account.current()\` (returns the username or null). While logged in, the visitor's \`Sites.me.*\` data is tied to the ACCOUNT (portable across devices) instead of the anonymous cookie; these calls reject (throw) on taken usernames or bad credentials, so catch and show a message. The backend serves only after the site is deployed.
