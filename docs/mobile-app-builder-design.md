...
_Status: **Phases 0–3 (MVP) BUILT & live-verified** (2026-07-18). Owner: architecture. Extends the unified Artifacts feature (`src/lib/tools/artifacts.ts`, `src/components/artifacts/sandbox.ts`, `src/app/s/[slug]/route.ts`) from web-only to also building mobile apps._

> **Phase 3 MVP build note (2026-07-18):** a **Snack preview engine** ships for `mobile` artifacts — `src/components/artifacts/SnackEmbed.tsx` embeds Expo Snack's web player in a cross-origin iframe and delivers the App.tsx source over the same postMessage handshake `embed.js` uses (no third-party script in our origin, no URL-length limit). Snack runs the **real Expo runtime + Snackager**, so it resolves the richer RN libraries the local import map can't (react-navigation, safe-area-context, vector-icons — auto-extracted from the source), and is the same runtime that runs on-device. The panel gains a **Local | Snack** engine toggle (mobile-only) and a **"Run on device"** hand-off that opens the full Snack editor (QR + Expo-Go SDK reconciliation on Snack's side). Pure client — no new npm deps, no backend route, no app CSP change (the app sets no frame-src). **Scoped OUT of the MVP** (heavier / gated): inline device QR via `snack-sdk` live sessions (+ the reality that stock store Expo Go trails SDK 57), and an in-product EAS build service (stores a user Expo token + server-side `eas` — and Phase 2 already emits a project the user builds themselves). Live-verified: Snack toggle injects the exact artifact source and bundles with zero errors.
